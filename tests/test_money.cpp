...
TEST(Currency, RejectsUnknownCode) {
  EXPECT_FALSE(currencyFromCode("XXX").ok());
  EXPECT_EQ(currencyFromCode("XXX").error(), ErrorCode::UnknownCurrency);
  EXPECT_FALSE(currencyFromCode("usd").ok());  // 小寫不接受
  EXPECT_FALSE(currencyFromCode("").ok());
}
...
TEST(Currency, SameIntegerMeansDifferentMoney) {
  ...
  // 同一個 5000，差一百倍。任何寫死 /100 的程式碼都會在 JPY 上出錯。
}
...
TEST(Currency, FormatHandlesInt64Extremes) {
  // INT64_MIN 取相反數會溢位，所以格式化時必須繞過去（見 Currency.cpp）。
  // 這裡不驗確切字串，只確認它不會崩、不會是空的。
...
TEST(Money, RefusesToMixCurrencies) {
  ...
  EXPECT_EQ(usd.plus(jpy).error(), ErrorCode::CurrencyMismatch);
  EXPECT_EQ(usd.minus(jpy).error(), ErrorCode::CurrencyMismatch);
}

TEST(Money, ReportsOverflowInsteadOfWrapping) {
  const Money big{kMaxAmount, Currency::USD};
  EXPECT_EQ(big.plus(Money{1, Currency::USD}).error(), ErrorCode::AmountOverflow);

  const Money small{kMinAmount, Currency::USD};
  EXPECT_EQ(small.minus(Money{1, Currency::USD}).error(), ErrorCode::AmountOverflow);
  EXPECT_EQ(small.negated().error(), ErrorCode::AmountOverflow);
}

TEST(Money, ToStringIncludesCurrencyCode) {
  EXPECT_EQ(Money(5000, Currency::USD).toString(), "50.00 USD");
  EXPECT_EQ(Money(5000, Currency::JPY).toString(), "5000 JPY");
}
